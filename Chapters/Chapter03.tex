...
\begin{remark}
    ...
    We see a loop \(\gamma\) in the polytope \(P_i\), being reflected in faces.
    ...
    \begin{figure}[h!]
        \label{fig:ray}
        \centering
        \includegraphics[width=.75\textwidth]{gfx/Ray bouncing in P.png}
    \end{figure}
    ...
    On the other hand, if we consider the loop \(\gamma^2\), i.e. the loop \(\gamma\) gets run through twice, then \(\gamma^2\) hits the face \(F_i\) twice.
    By our reasoning above, we should see a loop coming from \(\gamma\) in the two-fold cover \(P_{i+1}\).
...
We will mentioned it in the following remark, without giving details.
...
\begin{remark}
    ...
    \begin{figure}[h!]
        \label{fig:badray}
        \centering
        \includegraphics[height=3.3cm]{gfx/Equivalent loops.png}
    \end{figure}
    ...
    In fact, we can always find a loop \(\gamma'\), homotopy equivalent to \(\gamma\), that is only hitting codimension-\(1\) faces.
\end{remark}
...
For us it will be important that the generic projection of \(\gamma\) down to the orbifold \(P_i\) implies that in the neighborhood \(N\), coming from the two-sidedness the ray \(\gamma\) is injective, i.e. we don't just walk to the face and go back without getting reflected. % TODO: check this!
