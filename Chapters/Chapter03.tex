...
We should make clear what we have to show.
...
By \Cref{rmk:groupseries}, the cofinal two-fold tower induces a descending series of fundamental groups of the polytopes \(P_i\) and intersecting them with the finite index subgroup \(W'\) will not allow them to grow uncontrollably.
Thus, the descending series of subgroups formed by the groups \(W_i\) is again cofinal.
...
\end{enumerate}
The first condition is essentially a consequence of the fact that the series of orbifold fundamental groups has index precisely two in each step.
...
Fix a loop \(\gamma\) that is a representative of \(w\) in the manifold \(\faktor{WC}{W_i}\).
...
By our choice of the element \(w\), what we are seeing is that \(\gamma\) hits this face \(F_i\) an odd number of times.
If it would hit \(F_i\) an even number of times, \(\gamma\) would lift to a loop in \(P_{i+1}\) and therefore \(w\) would lift to the group \(W_{i+1}\), a contradiction.
...
\begin{definition}
    Let \(M\) be a manifold.
    A submanifold \(F \subset M\) is called \emph{two-sided} in \(M\), if it locally looks like the product with an intervall.
...
Indeed, the only point we have to think about is the red one. % TODO: the following is wrong - maybe put this in a remark
A neighborhood \(U := (\varepsilon - \delta, \varepsilon] \cup x[-\varepsilon, -\varepsilon + \delta)\) around it in the line (or the circle) is open, if and only if its preimage under the composed quotient map is open in the manifold \(M_i\).
...
A continuous map between two topological spaces induces a homomorphism between their fundamental groups, so that we get the desired morphism from \(\pi_1(M_i)\) to \(\pi_1(S^1) \cong \Z\).
