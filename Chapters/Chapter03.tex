...
Observe that in praxis, it suffices to show that conditions \(2\) to \(4\) hold. % as once they are satisfied we can simply pass to the Core of each \(G_i\) in \(G\) and this new sequence will.
...
Following \Cref{thm:cofinal}, we fix a cofinal doubling sequence \(P_k\), starting in the fundamental chamber \(P_0 = C\), i.e. a cofinal sequence of two-fold orbifold covers \(C \leftarrow P_1 \leftarrow P_2 \leftarrow \cdots\).
...
By \Cref{rmk:groupseries}, the groups \(W_i\) form a cofinal descending series of subgroups.
...
As we are in the situation that we have either \(\faktor{G_i}{G_{i+1}} \cong \faktor{\Z}{2\Z}\) or \(\faktor{G_i}{G_{i+1}} \cong \{1\}\), we conclude that the derived subgroup \(G_i^{(1)}\) is a subgroup of \(G_{i+1}\). %, due to the fact that \(G_i^{(1)}\) is a subgroup of the kernel of the isomorphism.
...
\begin{figure}[h!]
    \centering
    \begin{tikzcd}
        G_i \arrow[rr] \arrow[dr] & {} & \faktor{G_i}{G_{i+1}} \cong \faktor{\Z}{2\Z} \\
        {} & \faktor{(G_i)_{\text{ab}}}{\text{Torsion}} \arrow[ru, dashed]
    \end{tikzcd}
\end{figure}

Now, take an element \(g\) in \(G_i\) that is not contained in \(G_{i+1}\) and a representative loop \(\gamma\) of \(g\) in the manifold \(D_i'\).
